...
const usaDrive = typeof CONFIG !== 'undefined' && CONFIG.API_KEY && CONFIG.PASTA_DRIVE_ID;
...
async function listarDoDrive(){
  const q = `'${CONFIG.PASTA_DRIVE_ID}' in parents and mimeType='application/pdf' and trashed=false`;
  const u = 'https://www.googleapis.com/drive/v3/files'
    + '?q=' + encodeURIComponent(q)
    + '&key=' + CONFIG.API_KEY
    + '&fields=' + encodeURIComponent('files(id,name,thumbnailLink)')
  ...
  return (d.files || []).map(f => ({
    ...
    url: `https://www.googleapis.com/drive/v3/files/${f.id}?alt=media&key=${CONFIG.API_KEY}`,
